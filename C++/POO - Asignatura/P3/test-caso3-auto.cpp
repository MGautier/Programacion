...
FCT_BGN() {
  ...
  FCT_SUITE_BGN(LineaPedido) {
    FCT_TEST_BGN(LINEA_PEDIDO - valor por defecto en segundo parametro constructor) {
  ...
  FCT_FIXTURE_SUITE_BGN(Dos compras de dos usuarios) {
    ...
    FCT_SETUP_BGN() {
      ...
      pPed1 = new Pedido(*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, *pTarjetaU, fHoy);
    ...
    FCT_TEST_BGN(ARTICULO<->PEDIDO - ventas de un articulo) {
      const Pedido_Articulo::Pedidos& pedArticulo1 = pAsocPedidoArticulo->ventas(articulo1);
      if (pedArticulo1.size() == 2) {
        Pedido_Articulo::Pedidos::const_iterator it = pedArticulo1.find(const_cast<Pedido*>(pPed1));
        fct_chk_eq_dbl(it->second.precio_venta(), articulo1.precio());
    ...
    FCT_TEST_BGN(ARTICULO<->PEDIDO - insercion en flujo de Pedidos) {
      ...
      const double totalEsperado = (cantidad_A1_P1 + cantidad_A1_P2) * articulo1.precio();
    ...
    FCT_TEST_BGN(PEDIDO<->USUARIO - pedidos de un usuario) {
      const Usuario_Pedido::Pedidos& pedidosU2 = pAsocUsuarioPedido->pedidos(*pU2);
      if (pedidosU2.size() == 1) {
